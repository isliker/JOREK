...
function initial_run () {
  ...
  ${codedir}/util/setinput.sh input restart_particles=.f. restart=.t. nstep_n=10 tstep_n=10. use_fixed_rng_value=.f.      || exit 1
...
function restart_run () {
  ${codedir}/util/setinput.sh input restart_particles=.t. restart=.t. nstep_n=2 tstep_n=10. use_fixed_rng_value=.t.       || exit 1
  export OMP_NUM_THREADS=$num_threads                                                                                     || exit 1
  echo "setting OMP_NUM_THREADS=$num_threads, due to the requirements of the test"                                        || exit 1
