# --- General settings
jorekmodel="500"
description="Shattered pellet injection case, JET-like plasma, model$jorekmodel, n_tor=3."
mpitasks=2
...
function initial_run () {
  ${codedir}/util/setinput.sh input nstep_n=100,                                     || exit 1
  $MPIRUN 1 ./jorek_model${jorekmodel}_1 < input | tee logfile_initial               || exit 1
  ${codedir}/util/setinput.sh input nstep_n=230 restart=.t.                          || exit 1
...
function restart_run () {
  ${codedir}/util/setinput.sh input restart=.t. nstep_n=1 nout=1                     || exit 1
