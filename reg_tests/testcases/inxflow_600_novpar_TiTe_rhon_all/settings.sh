# --- General settings
jorekmodel="600"
options="with_vpar=.false. with_TiTe=.true. with_neutrals=.true. with_impurities=.false. with_refluid=.false."
description="Ballooning mode, simple X-point plasma, model$jorekmodel, two temperature, novpar, rhon, n_tor=3 + FFT, many terms switched on."
mpitasks=2
...
function compile_jorek () {
  ...
  ./util/config.sh model=$jorekmodel n_tor=3 n_coord_tor=1 l_pol_domm=0 n_plane=4 n_period=6 n_coord_period=1 $options || exit 1
  make $compilopt $debugoptions jorek_model${jorekmodel}                                                               || exit 1
...
function initial_run () {
  ...
  ${codedir}/util/setinput.sh input nstep_n=160 tstep_n=1.d1 restart=.t.             || exit 1
