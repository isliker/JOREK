...
if (.not. restart_particles) then
  ...
  !< Maxwellian distribution is initialised from the bulk plasma, but later the velocities should be rescaled
  !< to math the desired E_hot, so that v_hot = sqrt(E_hot/E_bulk)*v_bulk with E_hot taken on axis
